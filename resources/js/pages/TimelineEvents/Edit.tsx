// resources/js/pages/TimelineEvents/Edit.tsx
...
export default function TimelineEventEdit({ timelineEvent, mediaItems }: Props) {
  ...
  const { data, setData, put, processing, errors } = useForm({
    ...
    media_ids: timelineEvent.media?.map(m => m.id) || [] as number[],
    title: {
      en: timelineEvent.title?.en || '',
      ar: timelineEvent.title?.ar || '',
    },
    description: {
      en: timelineEvent.description?.en || '',
      ar: timelineEvent.description?.ar || '',
    },
  });
  ...
  useEffect(() => {
    ...
    if (timelineEvent.media) {
      setSelectedMedia(timelineEvent.media);
    }
  }, [timelineEvent.media]);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    put(`/timeline-events/${timelineEvent.id}`);
  };

  return (
    <AppLayout breadcrumbs={breadcrumbs}>
      <Head title={`Edit - ${timelineEvent.title?.en || 'Timeline Event'}`} />
      ...
      <div className="flex h-full flex-1 flex-col gap-4 rounded-xl p-4 overflow-x-auto">
        <div className="flex flex-col gap-4 sm:flex-row sm:justify-between sm:items-center">
          <div className="space-y-1">
            ...
            <p className="text-sm text-muted-foreground">
              Update timeline event details and content
            </p>
          </div>
          <div className="flex gap-2">
            <Badge variant="outline">{timelineEvent.timeline_event_id}</Badge>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 max-w-7xl">
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                  ...
                  <div className="flex justify-end gap-3 pt-4">
                    ...
                    <Button 
                      ...
                      className="gap-2"
                    >
                      {processing ? 'Updating...' : 'Update Timeline Event'}
                    </Button>
                  </div>
                </form>
              </CardContent>
          ...
          {/* Sidebar - Same as Create but with existing data */}
          <div className="space-y-4">
            {/* Preview */}
            <Card>
              <CardHeader className="px-6 py-4 border-b">
              ...
              <CardContent className="p-6">
                <div className="space-y-3 text-sm">
                  <div className="flex justify-between">
                    ...
                    <span className="font-mono">{timelineEvent.timeline_event_id}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">Created:</span>
                    <span>{new Date(timelineEvent.created_at).toLocaleDateString()}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">Updated:</span>
                    <span>{new Date(timelineEvent.updated_at).toLocaleDateString()}</span>
                  </div>
                </div>
              </CardContent>
            </Card>
